    ...
    <div class="app" hx-ext="sse" sse-connect="/events/stream">
      ...
      <div class="main">
        <header class="topbar">
          ...
          <div class="topbar-actions">${paletteTrigger()}${o.primaryAction ?? ""}<button type="button" x-data class="btn-ic theme-toggle" aria-label="Toggle light or dark theme" title="Toggle theme" @click="const next = document.documentElement.getAttribute('data-theme') === 'light' ? 'dark' : 'light'; document.documentElement.setAttribute('data-theme', next); document.cookie = 'ps_theme=' + next + '; path=/; max-age=31536000; samesite=lax';">${icon("sun", "ico ico-sun")}${icon("moon", "ico ico-moon")}</button><button class="btn-ic" type="button" hx-post="/logout" title="Sign out" aria-label="Sign out">${icon("logout", "ico")}</button></div>
        </header>
