...
| Secret | Value |
|---|---|
| `META_PROBE_APP_ID` / `META_PROBE_APP_SECRET` | Facebook app credentials for the dedicated test app. |
| `META_PROBE_PAGE_TOKEN` / `META_PROBE_PAGE_ID` | Page token and numeric ID for the dedicated test Page. |
| `META_PROBE_IG_ID` | Instagram business-account ID linked to the test Page. |
| `META_PROBE_IG_TOKEN` | Long-lived token generated by the test app's separate Instagram Business Login setup. |
| `META_PROBE_IG_USER_ID` | IGSID of a dedicated test recipient with an open messaging window. |
...
sends one Facebook test DM and one Instagram test DM. Before dispatching it, open fresh conversations
from dedicated test accounts to both the Page and Instagram account so the replies are policy-compliant.
The Facebook Page token is used only on `graph.facebook.com`; the Instagram Business Login token is
used only on `graph.instagram.com`. Rotate expiring test tokens before a strict run. A green run still
requires a maintainer to review Meta's changelog and execute the assisted inbound-webhook check before
merging its PR.
