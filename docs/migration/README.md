...
Moving from a hosted tool like Buffer, Hootsuite, Later, Publer, SocialBee, or ManyChat? Faina is
...
of them gives you a CSV) and push it into your Faina instance through the public `/api/v1` API. That
...
with **your export file**, your **Faina URL**, and an **API key**, and ask it to migrate you. The agent
...
| ManyChat / Chatfuel | Automations / flows | Rebuilt as Faina rules & sequences (flows are not exportable) | [rebuild-automations.md](rebuild-automations.md) |
...
- **ManyChat flows / automations** — ManyChat does not expose them as machine-readable data (export is only
  a PNG or a share link). You rebuild them as Faina rules and sequences — usually a 10-minute job, and
...
1. Open your Faina dashboard → **Settings → API keys**.
